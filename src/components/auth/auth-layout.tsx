...
export type AuthVariant = "sign-in" | "sign-up" | "verify";

const AUTH_HERO: Record<"sign-in" | "sign-up", { src: string; alt: string }> = {
  "sign-in": {
...
export function AuthLayout({ variant, children }: AuthLayoutProps) {
  const hero = variant !== "verify" ? AUTH_HERO[variant] : null;
  ...
  if (variant === "verify") {
    return (
      <div className="flex min-h-[dvh] flex-col items-center justify-center px-4 py-8">
        <div
          className={cn(
            "w-full max-w-[480px] rounded-card bg-card p-6 shadow-sm ring-1 ring-border",
            "sm:p-8"
          )}
        >
          {children}
