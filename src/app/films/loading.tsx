export default function FilmsLoading() {
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-8">
        <div className="h-10 w-48 animate-pulse rounded-card bg-muted" />
        ...
        <div className="w-full shrink-0 space-y-6 lg:w-56">
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i}>
              ...
              <div className="h-10 animate-pulse rounded-card bg-muted" />
            </div>
          ))}
        </div>
        ...
        <div className="min-w-0 flex-1">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 md:grid-cols-4 lg:grid-cols-5">
            {Array.from({ length: 10 }).map((_, i) => (
              <div key={i} className="overflow-hidden rounded-card border border-border/50">
